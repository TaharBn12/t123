...
initPage('استقبال المسح من الهاتف', null, async (c)=>{
  ...
  const render=()=>{ const L=U.qs('#list'); U.qs('#cnt').textContent=`${deviceCount} عملية مسح`; U.qs('#collectCount').textContent=items.length;
    ...
    L.innerHTML = items.map((it,i)=>{ const p=it.product; if(!p) return `<div class="scan-item unknown ${i===0?'new':''}"><div class="pimg">❓</div><div><b>باركود غير مسجل</b><div class="muted" style="direction:ltr;text-align:right">${U.esc(it.barcode)}</div><small class="muted">${U.date(it.at)}</small></div><div class="flex gap"><a class="btn sm success" href="product-form.html?barcode=${encodeURIComponent(it.barcode)}"><i class="fa-solid fa-plus"></i> إضافة كمنتج</a><button class="icon-btn" data-rm="${i}"><i class="fa-solid fa-xmark"></i></button></div></div>`;
      const diff = it.count - (+p.stock);
      return `<div class="scan-item ${i===0?'new':''}"><div class="pimg">${p.image_url?`<img src="${U.esc(p.image_url)}">`:'📦'}</div>
