initPage('المرتجعات', null, async (c)=>{
  ...
  const show=async(sale)=>{ const {data:prev}=await db.from('return_items').select('product_id,quantity,returns!inner(sale_id)').eq('returns.sale_id',sale.id); const done={}; (prev||[]).forEach(p=>done[p.product_id]=(done[p.product_id]||0)+ +p.quantity);
    ...
    U.qs('#doRet').onclick=async()=>{ const items=U.qsa('[data-i]').map(inp=>({item:sale.sale_items.find(x=>x.id===inp.dataset.i),qty:U.toNum(inp.value),max:+inp.max})).filter(x=>x.qty>0); if(!items.length) return U.toast('حدد كمية','info'); if(items.some(x=>!(x.max>0)||x.qty>x.max)) return U.toast('الكمية تتجاوز المتبقي من الفاتورة','error');
      const total=items.reduce((s,x)=>s+x.qty*x.item.unit_price,0); if(!await U.confirm(`إرجاع ${items.length} صنف بقيمة ${U.money(total)}؟`)) return;
      const {data:r,error}=await db.from('returns').insert({sale_id:sale.id,user_id:Auth.user.id,total,reason:U.qs('#reason').value}).select().single(); if(error) return U.toast(error.message,'error');
      await db.from('return_items').insert(items.map(x=>({return_id:r.id,product_id:x.item.product_id,quantity:x.qty,unit_price:x.item.unit_price,total:x.qty*x.item.unit_price})));
