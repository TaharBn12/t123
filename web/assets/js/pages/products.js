initPage('المنتجات', null, async (c)=>{
  ...
  const dt=new DataTable({el:U.qs('#tbl'),table:'products',select:'*,categories(name)',title:'قائمة المنتجات',search:['name','barcode'],order:'created_at',
    ...
    columns:[{label:'المنتج',render:r=>`<div class="flex gap"><div style="width:40px;height:40px;border-radius:8px;background:var(--bg);display:grid;place-items:center;overflow:hidden">${r.image_url?`<img src="${U.esc(r.image_url)}" style="width:100%;height:100%;object-fit:cover">`:U.imgPlaceholder('fa-box-open')}</div><div><b>${U.esc(r.name)}</b><br><small class="muted" style="direction:ltr">${U.esc(r.barcode||'')}</small></div></div>`,raw:r=>r.name},
